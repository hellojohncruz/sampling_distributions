...
knitr::kable(global_monitor_tbl)
```
...
The comparison bar graph shows that both the population and sampling proportions are similar.  

```{r samp1-graph, fig.height=2.5, fig.width=10}
samp1_tbl <- 
  samp1 %>%
  count(scientist_work) %>%
  mutate(sample_prop = n /sum(n))

knitr::kable(samp1_tbl)

samp1_rbind <- samp1_tbl |> 
  pivot_longer(cols = 'sample_prop', names_to = 'prop_type', values_to = 'prop')

global_rbind <- global_monitor_tbl |> 
  pivot_longer(cols = 'pop_prop', names_to = 'prop_type', values_to = 'prop')

rbind(global_rbind, samp1_rbind) |> 
  ggplot(aes(x = scientist_work, y = prop, fill = prop_type)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = scales::percent(prop), fontface = 'bold'), position = position_dodge2(width = 0.9), hjust = 1.1, vjust = 0.5) +
  coord_flip()
...
I would not expect the same sample proportion of another student. This is because of the randomization of each sample will change what the sample mean will be. The sample means should be similar to the population proportion. Checking in with another student, their sample mean was 0.26 (13/50). This is clearly different from what I found, yet the same percentage difference from the population proportion. 
...
Surprisingly, *samp2* happened to be exactly what the population proportion is. Compared to *samp1*, they are relatively similar to each other. The sample size of 1000 should provide a more accurate estimate of the population proportion as the sample size is getting closer to the total population.
...
There are 15,000 samples taken from the population. The sampling distribution is normally distributed and it's center is 0.20, which is what the population proportion is. 
...
There are 25 observations as we replicated 25 samples from the population. Each observation gives us a sample proportion that estimates what the population proportion is. 

```{r rep_sample_n-sample_props_small}
...
knitr::kable(sample_props_small)
...
Each observation in the sampling distribution represents the sampling proportion of each sample.

| sample size | mean | std_error |
| :---------- | :--- | :-------- |
|     10      | 0.22 |    0.11   |
|     50      | 0.20 |    0.06   |
|    100      | 0.20 |    0.04   |

We can see that as the sample size increase, the mean gets closer to the population mean and the standard error decreases. We also see the shape of the sampling distributions go from right-skewed, to normally distributed, to almost no tails, where the variation from the mean is low. As for increasing the number of simulations, it does change the mean and standard error if the sample size is small, however, the sample size itself has more of a effect on mean and standard error. 
...
Based on a sample size of 15, a close estimate for the population proportion is 86% of people think work scientists do enhances their lives. 
...
The sampling distribution looks left-skewed. Based on the distribution, the population mean should be roughly 0.75, given the skewness. We can verify if this is true based on the table provided, which is exactly 0.8.
    
```{r rep-benefits-sample15}
...
The sampling distribution looks normally distributed. Compared to the sample size of 15, we can see that the larger sample size minimizes any skewness. Based on this distribution we can see the mean being 0.8, which is the population proportion. 

```{r rep-benefits-sample150}
...
When the sample size is larger, we can see the spread to be smaller. If we want to be able to capture the true population, the smaller the spread, the closer we are to the population mean. 
